/*!
 * \file
 * \brief Class module::Monitor_BFER_AR.
 */
#ifndef MONITOR_BFER_AR_HPP_
#define MONITOR_BFER_AR_HPP_
...
class Monitor_BFER_AR : public Monitor
{
public:
...
protected:
	struct Attributes
	{
		std::atomic<unsigned long long> n_fra; // the number of checked frames
		std::atomic<unsigned long long> n_be;  // the number of wrong bits
		std::atomic<unsigned long long> n_fe;  // the number of wrong frames
...
private:
	...
	std::shared_ptr<Attributes> vals;
	tools::Histogram<int> err_hist; // the error histogram record
	bool err_hist_activated;

public:
	Monitor_BFER_AR(const int K, const unsigned max_fe, const unsigned max_n_frames = 0, const bool count_unknown_values = false);

	virtual ~Monitor_BFER_AR() = default;

	virtual Monitor_BFER_AR<B>* clone() const;

	bool equivalent(const Monitor_BFER_AR<B>& m, bool do_throw = false) const; // check if this monitor and "m" have equivalent construction arguments
	...
	virtual void collect(const Monitor_BFER_AR<B>& m, bool fully = false);
	virtual void collect(const Attributes& v);

	Monitor_BFER_AR<B>& operator+=(const Monitor_BFER_AR<B>& m); // not full "collect" call

	virtual void copy(const Monitor& m,         bool fully = false);
	virtual void copy(const Monitor_BFER_AR<B>& m, bool fully = false);
	virtual void copy(const Attributes& v);

	Monitor_BFER_AR<B>& operator=(const Monitor_BFER_AR<B>& m); // not full "copy" call
...
#include "Module/Monitor/BFER_AR/Monitor_BFER_AR.hxx"
#endif

#endif /* MONITOR_BFER_AR_HPP_ */
